...
app.post('/luxalgo', async (req, res) => {
  ...
  try {
    ...
    console.log(`🎯 [LuxAlgo] Executed! ${normalizedPair} | OANDA ID: ${result.tradeId} | Fill: ${result.fillPrice}`);
    ...
    return res.status(500).json({ error: 'LuxAlgo signal processing failed', detail: err.response?.data || [err.me](https://err.me)ssage });
  }
});
      ...
      if (proximity <= 0.0020) {
        try {
          await axios.post(BASE44_UPDATE_TRADE_URL, { id: setup.id, status: 'Triggered' },
            { headers: { 'x-api-key': BASE44_SERVICE_TOKEN, 'Content-Type': 'application/json' } });
          const result = await executeTrade({
            symbol: rawSymbol, action: setup.action,
            entry: parseFloat(setup.entry), stop_loss: parseFloat(setup.stop_loss),
            take_profit: parseFloat(setup.take_profit), comment: `TraderJoe Auto | ${rawSymbol} ${setup.action}`
          });
          await axios.post(BASE44_UPDATE_TRADE_URL,
            { id: setup.id, status: 'Filled', trade_id_oanda: result.tradeId,
              notes: (setup.notes || '') + ` | Auto-filled at ${result.fillPrice}` },
            { headers: { 'x-api-key': BASE44_SERVICE_TOKEN, 'Content-Type': 'application/json' } });
          console.log(`✅ [Poller] Filled ${rawSymbol}. ID: ${result.tradeId}`);
          fired++;
        } catch (execErr) {
          ...
          await axios.post(BASE44_UPDATE_TRADE_URL,
            ...
            { headers: { 'x-api-key': BASE44_SERVICE_TOKEN, 'Content-Type': 'application/json' } }).catch(() => {});
        }
      }
    }
    if (fired === 0) console.log('🔍 [Poller] Nothing close enough.');
  } catch (err) {
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'Execution failed', detail: err.response?.data || [err.me](https://err.me)ssage });
  }
});
...
app.post('/close-all', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: err.response?.data || [err.me](https://err.me)ssage });
  }
});
...
app.get('/status', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      balance: acct.data.account.balance, unrealized_pnl: acct.data.account.unrealizedPL,
      open_trades: trades.data.trades.length,
      trades: trades.data.trades.map(t => ({
        id: t.id, instrument: t.instrument, units: t.currentUnits, open_price: t.price, pnl: t.unrealizedPL
      }))
    });
  } catch (err) {
    return res.status(500).json({ error: err.response?.data || [err.me](https://err.me)ssage });
...
app.listen(PORT, () => {
  console.log(`🚀 Trader Joe on port ${PORT} | Max trades: ${MAX_CONCURRENT_TRADES}`);
  console.log(`📡 LuxAlgo endpoint active: POST /luxalgo`);
  pollTradeSetups();
  setInterval(pollTradeSetups, POLL_INTERVAL_MS);
  console.log(`⏱️  Poller active — every 5 minutes`);
});